mutable struct PhononDefectSolver{MT, VT} <: AbstractSolver
    iter :: Int
...
function PhononDefectSolver(model, ωs_, ks_, qpts; occupation, ωs_dense = nothing, ks_dense = nothing, η, Σs_rest = nothing)
    ...
    As0_dense = zeros(length(ωs_dense), length(ks_dense))
    spectral_sum = zeros(length(ks_dense))
    spectral_occ = zeros(length(ks_dense))
    D0 = zeros(ComplexF64, length(ωs_dense), length(ks_dense))
  ...
  #  end
    ...
    PhononDefectSolver(0, model, η, occupation, qpts, ωs, ks, ωs_dense, ks_dense,
        Σs, As, Σs_dense, As_dense, As0_dense, Σs_rest, spectral_sum, spectral_occ,
         q_integral_matrix, D0)
end

function Base.show(io :: IO, S :: PhononDefectSolver)
    print(io, "PhononDefectSolver(", S.model, ", η = ", S.η, ")\n")
